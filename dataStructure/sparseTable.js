class SparseTable { //js7777 SET
    ...
    constructor(a,op) {
        ...
        for(var i=2;i<=this.n;i++) this.lg[i]=this.lg[i>>1]+1
        this.k=this.lg[this.n]
        this.t=Array.from({length:this.k+1},()=>Array(this.n))
        for(var i=0;i<this.n;i++) this.t[0][i]=a[i]
        for(var j=1;j<=this.k;j++) for(var i=0;i+(1<<j)<=this.n;i++) this.t[j][i]=op(this.t[j-1][i],this.t[j-1][i+(1<<(j-1))])
    }
    Q(l,r) {
        var j=this.lg[r-l+1]
